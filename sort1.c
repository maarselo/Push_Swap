...
int ft_calculate_costb(t_stack *stack_b, t_node *node) //cambiar el orden
{
    ...
    if ((node->index > max->index) || (node->index < min->index))
    {
        if (stack_b->top == max)
            return (cost_b);
        if ((ft_getpos(max, stack_b) - 1) > (stack_b->size / 2))
            cost_b = cost_b + (stack_b->size - (ft_getpos(max, stack_b) - 1));
    }
   else
    {
        tmp = stack_b->top;
        min = tmp; //like min prev
        while (tmp)
        {
            if ((tmp->value < node->value) && (ft_abs(tmp->index - node->index) < ft_abs(min->index - node->index)))
                min = tmp;
            tmp = tmp->next;
        }
        printf("nigga:%d", min->value);
        cost_b = ft_getpos(min,stack_b);////////////////
        if (cost_b > ft_check_size(stack_b))
            cost_b = stack_b->size - cost_b + 1;
        else
            cost_b -= 1;
    }
    return (cost_b);
...
t_node *ft_find_cheapest_node(t_stack *stack_a, t_stack *stack_b )
{
    ...
    current = stack_a->top;
    cheapest = current;
    min_cost = ft_calculate_costa(stack_a, current) + ft_calculate_costb(stack_b, current);
    while(current)
    {
        cost = ft_calculate_costa(stack_a, current) + ft_calculate_costb(stack_b, current);
        if (cost < min_cost)
        {
            min_cost = cost;
            cheapest = current;
...
void    ft_push_to_stackb(t_stack *stack_a, t_stack *stack_b, t_node *cheapest)
{
    ...
    {
        if ((ft_getpos(cheapest, stack_a) <= (ft_check_size(stack_a) / 2)) && (cost_b <= (ft_check_size(stack_b) / 2)))
        {
            ft_rr(stack_a, stack_b);
            cost_a--;
            cost_b--;
        }
        else if ((ft_getpos(cheapest, stack_a) > (ft_check_size(stack_a) / 2)) && (cost_b > (ft_check_size(stack_b) / 2)))
        {
    ...
    while (cost_a > 0)
    {
        if (ft_getpos(cheapest, stack_a) <= (ft_check_size(stack_a) / 2))
            ft_rotate(stack_a, 'a');
        else
    ...
    while (cost_b > 0)
    {
        if (cost_b <= (ft_check_size(stack_b) / 2))
            ft_rotate(stack_b, 'b');
        else
...
void    ft_rotate_b_to_push(t_stack *stack_b)
{
    t_node *max;
    ...
    {
        if (ft_getpos(max, stack_b) <= (ft_check_size(stack_b) / 2))
            ft_rotate(stack_b, 'b');
        else
            ft_reverse(stack_b, 'b');
    }
...
*/
    ft_push(stack_a, stack_b, 'b');
    ft_push(stack_a, stack_b, 'b');
    printf("\nmarcelo: %d\n", ft_calculate_costb(stack_b, stack_a->top));
    ft_print_stacks(stack_b);
    ft_push(stack_a, stack_b, 'b');
    ft_sorted_three(stack_b);
    printf("\nmarcelo: %d\n", ft_calculate_costb(stack_b, stack_a->top));
    ft_print_stacks(stack_b); t_stack *stack_c = (t_stack*)malloc(sizeof(t_stack));
    ft_push(stack_a, stack_b, 'b');
    ft_sorted_four(stack_b, stack_c);
    printf("\nmarcelo: %d\n", ft_calculate_costb(stack_b, stack_a->top));
    ft_print_stacks(stack_b);
}
